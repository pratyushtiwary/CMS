...
export default function Complaints(props){
	...
	return (
		<>
			...
			<Header
				title = "Complaints"
				items = {["Home","New User","New Announcement","New Department","Announcements","Departments","Users","Settings"]}
				links = {["/","/new_user","/new_announcement","/new_department","/announcements","/departments","/users","/settings"]}
				icons = {["home","person_add","notification_add","plus_one","campaign","domain","people","settings"]}
				hideNewComplaint
			/>
			...
			<div className={styles.cont}>
				...
				<div className={styles.complaints}>
					{
						complaints && loaded===true && complaints.map((e,i)=>(
							<Card className={styles.complaint} key={i} variant="outlined">
								<CardActionArea className={styles.inner} href={"/complaint/"+e.id}>
									<Typography variant="h4" className={styles.title}>{e.shortDesc}</Typography>
									<Typography variant="subtitle1" className={styles.meta} title="By Employee"><Icon>person</Icon>{e.byEmp}</Typography>
									{
										e.assignedTo && (
											<Typography variant="subtitle1" className={styles.meta} title="Assigned To"><Icon>badge</Icon>{e.assignedTo}</Typography>
										)
									}
									{
										e.assignedOn && (
											<Typography variant="subtitle1" className={styles.meta} title="Assigned On"><Icon>schedule</Icon>{e.assignedOn}</Typography>
										)
									}
									<Typography variant="subtitle1" className={styles.meta+" "+styles.block+" "+styles[e.status]} title="Status"><Icon>grading</Icon>{e.status}</Typography>
									<Typography variant="subtitle1" className={styles.meta+" "+styles.block+" "+styles[e.priority]} title="Priority"><Icon>priority_high</Icon>{e.priority}</Typography>
									{
										e.rating && (
											<Typography variant="subtitle1" className={styles.meta} title="Average Feedback Rating"><Icon>star</Icon>{e.rating}/5</Typography>
										)
									}
								</CardActionArea>
							</Card>
				...
				</div>
				{
					loaded===true && more && !Boolean(loadingMsg) && (
						<Button variant="outlined" color="primary" className={styles.loadMore} onClick={searching?searchMore:loadMore}>Load More</Button>
					)
				}
				{
